...
//Description: This utility creates randomly
// generated maps based on user input.
...
struct tile // For specific tiles on the map. Ex. "Mountains - Cave"
{
	string tile_name;
	string type_name;
};
...
void create_tiles(tile_type tile[], int& temp_num_custom_tiles) // TILE CREATION! - MAIN INPUT...
{
	int i = 0; // set counter
...
void edit_tile_types(tile_type tile[], int& temp_num_custom_tiles)//SECONDARY INPUT
{
	int tile_selection;
	char tile_attribute;
	...
	for(i = 0; i != temp_num_custom_tiles; i++)
	{

		cout << setw(2) << i << " --- NAME - " << tile[i].type_name << " --- CHARACTER/COLOR - {";
		std::cout << "\033" << "[" << 0 << ";" << tile[i].color << "m" << tile[i].letter  <<  "\033" << "[m";
	...
	{
		cout << setw(2) << tile_selection << " --- NAME - " << tile[tile_selection].type_name << " --- CHARACTER/COLOR - {";
		std::cout << "\033" << "[" << 0 << ";" << tile[tile_selection].color << "m" << tile[tile_selection].letter  <<  "\033" << "[m";
		cout << "}\n"; //displays the tile the user entered
...
void mapgen(tile_type tile[], int& temp_num_custom_tiles)//MAIN OUTPUT
{
	...
	tile_type map[1000][1000]; // This is the actual map.
	...
	cout << "Please enter map length (1-1000): "; //Actual maxes depend on the system and hardware.
	cin >> max_x;
	...
	for(tile_selection = 0; tile_selection < temp_num_custom_tiles; tile_selection++) // Used for debugging. Will keep for awhile.
	{
		cout << setw(2) << tile_selection << " --- NAME - " << tile[tile_selection].type_name << " --- CHARACTER/COLOR - {";
		std::cout << "\033" << "[" << 0 << ";" << tile[tile_selection].color << "m" << tile[tile_selection].letter  <<  "\033" << "[m";
		cout << "}  ID: " << tile[tile_selection].ID << "\n";
		cout << "   --- BASE - " << setw(4) << tile[tile_selection].base_chance << " --- CHANCE FACTOR - " << tile[tile_selection].chance_factor << "\n";
	}
	//////////////////////////// 
	...
	while(mapy != (max_y)) // This is for having different rows
	{
		while(mapx != (max_x)) // Moving one tile at a time..
		{
			...
			// Displays the tile!
			std::cout << "\033" << "[" << 0 << ";" << map[mapx][mapy].color << "m" << map[mapx][mapy].letter  <<  "\033" << "[m";
...
void load_tiles(tile_type tile[], int& temp_num_custom_tiles)// Read below...
{
	string file_name;
	string file;
